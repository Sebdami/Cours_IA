...
namespace crea
{
	...
	bool Map::loadFromFileJSON(string& _filename)
	{
		...
		int i = 0; 
		int j = 0;
		for (Json::Value a : layers[0]["data"])
		{
			m_Grid[i][j]->setTileTerrainId(a.asInt());
			i++;
			if (i >= iWidth)
			{
				j++;
		...
		m_pTerrainTileSet->m_nImagewidth = tileSet1["imagewidth"].asInt();
		m_pTerrainTileSet->m_nMargin = tileSet1["margin"].asInt();
		m_pTerrainTileSet->m_szName = tileSet1["name"].asString();
		m_pTerrainTileSet->m_nSpacing = tileSet1["spacing"].asInt();
		m_pTerrainTileSet->m_nTilecount = tileSet1["tilecount"].asInt();
		...
		for (Json::Value a : tileSet1["terrains"])
		{
			...
			t->m_szName = a["name"].asString();
			t->m_szName = a["tile"].asInt();
